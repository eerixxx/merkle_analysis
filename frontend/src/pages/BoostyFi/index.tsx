...
export function BoostyFiPage() {
  ...
  const { data: stats, isLoading: statsLoading } = useQuery<BoostyFiStats>({
    queryKey: ['boostyfi', 'stats'],
    queryFn: boostyfiApi.getStats,
  })
  
  // Load root users when not focused on a specific user
  const { data: rootsData, isLoading: treeLoading } = useQuery<RootsResponse<BoostyFiUserTree>>({
    queryKey: ['boostyfi', 'roots', expandLevel, offset],
    queryFn: () => boostyfiApi.getRoots(expandLevel, limit, offset),
    enabled: focusedUserId === null,
  })
  
  // Load focused user's tree with ancestors
  const { data: focusedUserData, isLoading: focusedLoading } = useQuery<AncestorsResponse<BoostyFiUserTree>>({
    queryKey: ['boostyfi', 'ancestors', focusedUserId],
    queryFn: () => boostyfiApi.getAncestors(focusedUserId!),
    enabled: focusedUserId !== null,
  })
  
  // Load focused user's subtree
  const { data: focusedUserTree, isLoading: focusedTreeLoading } = useQuery<BoostyFiUserTree>({
    queryKey: ['boostyfi', 'tree', focusedUserId],
    queryFn: () => boostyfiApi.getUserTree(focusedUserId!, 3),
    enabled: focusedUserId !== null,
  })
  ...
  const { data: selectedUser, isLoading: userLoading } = useQuery<BoostyFiUser>({
    queryKey: ['boostyfi', 'user', selectedUserId],
    queryFn: () => boostyfiApi.getUser(selectedUserId!),
  ...
  const statsCards = stats ? [
    ...
    { title: 'Total ATLA', value: Number(stats.total_atla).toLocaleString(), icon: Coins },
